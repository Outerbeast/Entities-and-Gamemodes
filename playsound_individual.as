...
void PatchAmbientGeneric()
{
    ...
    {
        if( pSound is null || !pSound.pev.SpawnFlagBitSet( 1 << 6 ) )
            continue;
        ...
        g_EntityFuncs.CreateEntity( "trigger_script", dictPsi, true );
    ...
    switch( useType )
    {
        ...
        case USE_TOGGLE:
        {
            if( !bool( pTriggerScript.GetUserData( "is_sound_playing" ) ) )
            {
                ...
                g_Scheduler.SetTimeout( "PlaySoundInvidual", 0.0f, kvTriggerScript.GetKeyvalue( "$s_sound" ).GetString(), volume, channel );
