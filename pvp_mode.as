...
final class PvpMode
{
    ...
    void AssignTeam()
    {
        for( int playerID = 1; playerID <= g_Engine.maxClients; ++playerID )
        {
            ...
            if( pPlayer !is null )
            {
                if( pPlayer.IsAlive() && !_IS_ASSIGNED[playerID] )
                {
    ...
    void SpawnProtection(CBasePlayer@ pPlayer)
    {
        if( pPlayer !is null )
        {
            ...
            if( pPlayer.m_iClassSelection == 0 )
            {
                ...
                g_EngineFuncs.ServerPrint( "-- Player: " + pPlayer.pev.netname + " with targetname: " + pPlayer.GetTargetname() + " is Spectating (no free player slots available!)\n" );
            }
        }
